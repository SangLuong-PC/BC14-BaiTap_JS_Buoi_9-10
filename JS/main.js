...
document
  ...
  .addEventListener("click", handleUpdateStaff);
    ...
    if (staff) {
      ...
      if (findStaff) {
        ...
        if (listStaffFinded) {
          ...
          var lastAccountFind =
            listStaffFinded[listStaffFinded.length - 1].account;

          if (
            account === lastAccountFind &&
            (listStaffFinded.length - 1) % 2 === 0 &&
            classification !==
              listStaffFinded[listStaffFinded.length - 1].classification
          ) {
            pwdPage -= 1;
          }
        }
...
function delegationTable(e) {
  ...
  if (action === "change") {
    ...
    var staff = staffManager.listStaff.find(
